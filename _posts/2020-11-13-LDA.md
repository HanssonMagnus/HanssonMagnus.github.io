...
$$\mathcal{D} = \left{ \bold{w}_1, \bold{w}_2, \dots, \bold{w}_M \right}.

* $$N$$ number of words in each document denoted by
$$\bold{w} = \left( w_1, w_2, \dots, w_N \right), $$w_n$$ is the n:th word. Document i has $$N_i$$
words.

* A word is a vector with the same length as the vocabulary indexed by
$$\left{ 1, \dots, V \right}$$. The v:th word in the vocabulary is represented by a word vector
...
Depending on what paper you read the notation is a bit different, but it is useful go get an
understandint of all the variables, and parameters in the model.
...
$$\bold{w}_i$$, with length $$N_i$$, in a corpus $$\mathcal{D}$$,
...
Thus, each word is choosen by first picking a topic according to the topic distribution of the
...
<img src="/images/lda_plate.jpg" width="500">

(Note that the subscript of $$N_i$$ is dropped in the plate notation).
...
take on the values either $$0$$ or $$1$$. Thus, the outcome space is $$\left{ (0,0), (0,1), (1,0),
(1,1) \right}$$. However, if we do not know the probabilities for these four events, but we know the
...
2. Sample $$A_1 \sim P(A | B_0)$$ and $$B_1 \sim P(B | A_1).
...
$$
P(z_i = j | \bold{z}_{-i}, w_i, d_i) \propto \frac{C_{w_i j}^{WT} + \beta}{\sum_{w=1}^W
C_{wj}^{WT} + W \beta} \frac{C_{d_i j}^{DT}+\alpha}{\sum_{d_i t}^{DT} + T \alpha}
$$

where $$\bold{C}^{WT}$$ and $$\bold{C}^{DT}$$ are count matrices of the number of times word $$w$$
